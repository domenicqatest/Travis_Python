...
class TestAmpURLs(object):
    ...
    def test_send_from_excel(self):
        ...
        for i in xrange(count):  # Number of URLs = 72
            rownum = (i + 1)
            is_pass = worksheet.cell((rownum), 1).value

            if is_pass == "PASS":
                pass_count += 1
        header = "PASSES - {}".format(pass_count), " FAILURES - {}".format(count - pass_count)
        print header
        ...
        for i in xrange(71): # Number of URLs = 71
            ...
            time.sleep(1)
            #if driver.find_element_by_id("LPRemovePreview").is_displayed():
                #driver.find_element_by_id("LPRemovePreview").click()
            #else:
                #pass
